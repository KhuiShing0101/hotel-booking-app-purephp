<?php
    ...
    if (isset($_GET["id"])) {
        $id                           = (int)($_GET["id"]);
        $sql                          = "SELECT name,type,occupancy,bed_id,size,view_id,description,detail,price_per_day,extra_bed_price_per_day FROM $table WHERE id = '$id'";
        $res_sql                      = $mysqli->query($sql);
        $res_row                      = $res_sql->num_rows;
        
        $special_feature_sql          = "SELECT special_features_id FROM $table_special_feature_id WHERE room_id = '$id'";
        $result_special_feature       = $mysqli->query($special_feature_sql);

        while ($row_special_feature = $result_special_feature->fetch_assoc()) {
            $special_features[]       = $row_special_feature["special_features_id"];
        }

        $amenities_sql                = "SELECT amenities_id FROM $table_room_amenities WHERE room_id = '$id'";
        $result_amenities_sql         = $mysqli->query($amenities_sql);
        
        while ($row_amenities = $result_amenities_sql->fetch_assoc()) {
            $amenities[]              = $row_amenities["amenities_id"];
        }
        ...
        $check_database_existence  = checkDatabaseExistence($mysqli, $table, $room_name, $id);
        ...
        if ($proc_error == false) {
            ...
            $res_del_sql                  = hardDelete($mysqli, $table_special_feature_id, $id);
            ...
            $res_delete_sql               = hardDelete($mysqli, $table_room_amenities, $id);
            ...
            $url                          = $cp_base_url . "show_room.php?message=update";
            header("Location:" . $url);
            exit();
        }
    } else {
        ...
        <div class="">
            <div class="row">
                <div class="col-md-12 col-sm-12">
                    <div class="x_panel">
                        <div class="x_content">
                            <form class="" action="<?= $cp_base_url ?>edit_room.php" method="post" novalidate>
                                <span class="section">Edit Hotel Room</span>
